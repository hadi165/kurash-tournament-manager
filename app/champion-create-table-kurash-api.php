...
require_once __DIR__ . '/match-advance.php';

header('Content-Type: application/json');

/**
 * This endpoint deletes every existing match for a weight category before it
 * regenerates, so it must never be reachable without a signed-in supervisor.
 *
 * validate-online.php is deliberately NOT used here: it answers with a
 * Location redirect, which a JSON client would follow into an HTML login page
 * and then fail to parse. An API says 401.
 */
if (empty($_SESSION['logged_in'])) {
    http_response_code(401);
    exit(json_encode(['status' => 'error', 'message' => 'Not signed in.']));
}

if (!in_array($_SESSION['user_role'] ?? '', ['admin', 'supervisor'], true)) {
    http_response_code(403);
    exit(json_encode(['status' => 'error', 'message' => 'Access denied. Admin or supervisor only.']));
}

if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    http_response_code(405);
    header('Allow: POST');
    exit(json_encode(['status' => 'error', 'message' => 'This endpoint changes data and requires POST.']));
}
...
// Main execution
try {
    $champion_id = filter_input(INPUT_POST, 'champion_id', FILTER_VALIDATE_INT);
    $championsub_id = filter_input(INPUT_POST, 'championsub_id', FILTER_VALIDATE_INT);

    if (!$champion_id || !$championsub_id) {
        throw new Exception('champion_id and championsub_id are required and must be integers.');
    }

    // Regenerating discards results. Refuse once any fight has been decided,
    // unless the caller explicitly confirms — a mis-click here mid-event would
    // erase every score already recorded for the category.
    $check = $pdo->prepare("
        SELECT COUNT(*) FROM championplaytablekurash
        WHERE champion_id = ? AND championsub_id = ?
          AND winner_athleteid IS NOT NULL AND wintype != 'bye'
    ");
    $check->execute([$champion_id, $championsub_id]);
    $decided = (int)$check->fetchColumn();

    if ($decided > 0 && empty($_POST['confirm_discard_results'])) {
        http_response_code(409);
        exit(json_encode([
            'status' => 'error',
            'message' => "This category already has {$decided} decided fight(s). "
                . "Regenerating the bracket will erase them. Re-send with confirm_discard_results=1 if that is intended.",
    ...
    // Delete-then-generate is one operation. A failure partway through used to
    // leave the category with no bracket at all.
    $pdo->beginTransaction();
    try {
        deleteExistingMatches($pdo, $champion_id, $championsub_id);
        createMatches($pdo, $champion_id, $championsub_id);

        // Byes are already decided the moment they are created, so their
        // winners have to be pushed into round two before anyone fights.
        $byesAdvanced = kurash_advance_all_byes($pdo, $champion_id, $championsub_id);
