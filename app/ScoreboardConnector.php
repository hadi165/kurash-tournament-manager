...
class ScoreboardConnector
{
    ...
    public function pushMatchToScoreboard(array $match): bool
    {
        ...
        // CURRENT_TIMESTAMP, not NOW() — SQLite has no NOW(), so this statement
        // used to throw and take the whole push down with it. CURRENT_TIMESTAMP
        // is also valid on MySQL/MariaDB and Postgres for the eventual port.
        $stmt = $this->pdo->prepare("
            UPDATE championplaytablekurash
            SET scoreboard_synced_at = CURRENT_TIMESTAMP
            WHERE id = ?
        ");
    ...
    public function applyResultToMatch(string $playCode, array $result): bool
    {
        ...
        // Refuse to mark a match complete without a winner. A malformed payload
        // that closed a fight with an empty winner would silently strand the
        // rest of that half of the bracket.
        if ($winnerSide !== 'a' && $winnerSide !== 'b') {
            error_log("ScoreboardConnector: rejected result for play_code {$playCode} — winner_side must be 'a' or 'b'");
            return false;
        }

        $athleteOnWinningSide = $match['athleteid_' . $winnerSide] ?? null;
        if (empty($athleteOnWinningSide)) {
            error_log("ScoreboardConnector: rejected result for play_code {$playCode} — no athlete on side {$winnerSide}");
            return false;
        }
        ...
        // Writing the result and advancing the winner must both happen or
        // neither: a bracket with a completed match and an empty next-round
        // slot is worse than one with no result at all.
        ...
        try {
            $stmt = $this->pdo->prepare("
                ...
                SET score_a = ?, score_b = ?, wintype = ?,
                    ...
                    court_status = 'completed', scoreboard_synced_at = CURRENT_TIMESTAMP
                WHERE id = ?
            ");
            $stmt->execute([
                $result['score_a'] ?? null,
            ...
            // Carry the winner into the next round. Without this the bracket
            // stops dead after round one, however many results arrive.
            kurash_advance_winner($this->pdo, array_merge($match, $winnerFields));

            $this->pdo->commit();
        } catch (Throwable $e) {
            $this->pdo->rollBack();
            error_log("ScoreboardConnector: failed to apply result for play_code {$playCode}: " . $e->getMessage());
            throw $e;
        }
