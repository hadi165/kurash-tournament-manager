...
// --- Auto-read the confirmed weigh-in CSV from disk (outside app/) ---
...
if (is_file($confirmedFile)) {
    ...
    $autoReadNote = "No confirmed file found at weigh-in-list-confirmed/{$fileNameSafe}.csv yet — go to Weigh-in Form, filter this weight category, and click \"Export Excel (Confirmed List)\" first.";
}
...
<div class="container">
    ...
    <?php if ($autoReadNote): ?>
        <!-- debug only, not shown to users: <?php echo htmlspecialchars($autoReadNote); ?> -->
    <?php endif; ?>
...
saveBtn.addEventListener('click', async function () {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            links.innerHTML =
                '<a href="file-download-kurash.php?folder=primary&file=' + encodeURIComponent(data.saved_as + '.html') + '&view=1" target="_blank">Open saved result (HTML/PDF)</a>' +
                ' &nbsp;|&nbsp; ' +
