...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_athlete_id'])) {
    ...
    if ($fullname !== '' && $noc_code !== '' && in_array($gender, ['M', 'F'], true) && isset($weightIds[$weight_index])) {
        // Scoped to the championship in the URL, not just the row id — an id
        // posted from a form is caller-supplied and must never reach across
        // championships.
        $stmt = $pdocon->prepare("
        ...
        $stmt->execute([
            ...
            $editId, $champion_id, $championsub_id
        ]);
    }
...
<?php require './page-header-kurash.php'; ?>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td colspan="7">
                    <form method="POST" action="" style="display:grid; grid-template-columns: repeat(3,1fr); gap:10px; align-items:end;"><?php echo csrf_field(); ?>
