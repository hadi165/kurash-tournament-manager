...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['athlete_id'])) {
    ...
    if (!$athlete_id || $weighin_value === false || $weighin_value === null) {
        ...
        // Scoped to the championship in the URL — see registration-kurash.php.
        $stmt = $pdocon->prepare("
            SELECT corashweight_text FROM championregisterathletes
            WHERE id = ? AND champion_id = ? AND championsub_id = ?
        ");
        $stmt->execute([$athlete_id, $champion_id, $championsub_id]);
        $athlete = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$athlete) {
            $message = 'That athlete is not registered in this championship.';
            $messageType = 'error';
        } else {
            ...
            $status = 'pending';
            if (preg_match('/^([+-]?)(\d+(?:\.\d+)?)/', trim($athlete['corashweight_text']), $m)) {
                $sign = $m[1];
                $limit = (float)$m[2];
                if ($sign === '+') {
            ...
            ");
            $stmt->execute([
                $weighin_value, $status, $weighin_value,
                $athlete_id, $champion_id, $championsub_id,
            ]);
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <form method="POST" action="" style="display:flex; gap:6px;"><?php echo csrf_field(); ?>
